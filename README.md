...
³ RWTH Aachen University \* Work done while visiting RWTH Aachen University
...
Turns out, _your ViT is secretly an image segmentation model_. EoMT shows that architectural complexity isn't necessary. For segmentation, a plain Transformer is all you need.
...
Then create a environment, activate it, and install the dependencies:
...
To export a ONNX from a pre-trained EoMT model, run:

```bash
python3 torch2onnx.py configs/dinov2/cityscapes/semantic/eomt_large_1024.yaml
```

The exported ONNX model will be saved in the `onnx` directory:

```bash
onnx/
└── cityscapes_semantic_eomt_large_1024.onnx
```
